            ...
            const ctx = gsap.context(() => {
                const tl = gsap.timeline({
                    scrollTrigger: {
                        ...
                        end: `+=${cards.length * 100}%`, // Scroll duration scales with card count
                ...
                const isShortScreen = typeof window !== 'undefined' && window.innerHeight < 768;
                const stackOffset = isShortScreen ? 15 : 30;
                const pushUpOffset = isShortScreen ? 10 : 20;
    ...
    return (
        <section ref={sectionRef} className="relative w-full h-screen bg-white font-noria overflow-hidden flex flex-col items-center justify-center py-10 [@media(max-height:768px)]:py-4">
            <div className="max-w-[1440px] w-full mx-auto px-4 md:px-12 flex flex-col items-center justify-center h-full">
                ...
                {/* Header */}
                <div className="mb-12 md:mb-16 w-full text-center shrink-0 [@media(max-height:768px)]:mb-6 [@media(max-height:650px)]:mb-3">
                    <h2 className="text-3xl md:text-5xl lg:text-6xl font-medium tracking-normal uppercase mb-4 [@media(max-height:768px)]:text-4xl [@media(max-height:650px)]:text-3xl">
                        What Doctors Gain with Joyzen
                    </h2>
                    <p className="font-satoshi text-xl md:text-3xl max-w-4xl leading-[1.2] mx-auto text-gray-800 [@media(max-height:768px)]:text-lg [@media(max-height:650px)]:text-base">
                        More time. Better structure. Stronger outcomes.
                    </p>
                </div>

                {/* Stacked Cards Container */}
                <div ref={containerRef} className="relative w-full h-[550px] sm:h-[450px] lg:h-[400px] [@media(max-height:768px)]:h-[350px] [@media(max-height:650px)]:h-[290px] max-w-[960px] mx-auto perspective-1000 shrink-0">
                    {cardData.map((card, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex flex-col sm:flex-row h-full p-8 md:p-12 [@media(max-height:768px)]:p-6 [@media(max-height:650px)]:p-4 gap-8 items-center relative z-10">
                                    {/* Left Text Side */}
                                    <div className="flex flex-col justify-center w-full sm:w-1/2">
                                        <span className={`font-satoshi text-6xl md:text-7xl lg:text-[100px] font-medium leading-none mb-4 md:mb-6 ${numColor} [@media(max-height:768px)]:text-5xl [@media(max-height:650px)]:text-4xl [@media(max-height:768px)]:mb-2`}>
                                            {card.num}
                                        </span>
                                        <h3 className={`text-3xl md:text-4xl lg:text-5xl font-semibold uppercase mb-4 ${titleColor} [@media(max-height:768px)]:text-2xl [@media(max-height:650px)]:text-xl [@media(max-height:768px)]:mb-2`}>
                                            {card.title}
                                        </h3>
                                        <p className={`font-satoshi text-xl md:text-2xl font-normal leading-snug ${descColor} [@media(max-height:768px)]:text-base [@media(max-height:650px)]:text-sm`}>
                                            {card.description}
                                        </p>
                                    </div>

                                    {/* Right Image Side */}
                                    <div className="w-full sm:w-1/2 h-full flex items-center justify-center sm:justify-end">
                                        <div className="relative w-full max-w-[200px] sm:w-auto sm:max-w-none sm:h-full max-h-[200px] sm:max-h-[280px] [@media(max-height:768px)]:sm:max-h-[210px] [@media(max-height:650px)]:sm:max-h-[170px] aspect-square rounded-3xl overflow-hidden shadow-xl bg-black/10">
                                            <Image
                                                src={card.image}
                                                alt={card.title}
                                                fill
